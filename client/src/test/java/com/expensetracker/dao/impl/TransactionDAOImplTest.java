 ...
 * 
 * These tests are disabled because they require an external database setup that
 * may not be available in all environments (CI/CD, local dev without DB, etc.).
 * 
 * The tests target database operations directly and would need:
 * - A running database instance (H2/MySQL configured)
 * - Proper test fixtures and cleanup between tests
 * - Potential data isolation concerns in shared test environments
 * 
 * For integration testing, consider:
 * 1. Using @TestInstance(Lifecycle.PER_CLASS) with shared H2 in-memory DB
 * 2. Running these tests in a dedicated integration test profile
 * 3. Using testcontainers for database isolation
 * 
 * To run these tests, remove @Disabled and ensure database is properly configured.
 */
@Disabled("Integration tests requiring database - see Javadoc for details")
